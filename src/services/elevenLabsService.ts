...
export const getModelCapabilities = (modelId: string): ModelCapabilities => {
  ...
  if (modelId.includes('_v1')) {
    return { ...defaults, supportsStyle: false, supportsSpeakerBoost: false };
  }
  if (modelId.includes('turbo')) {
    return { ...defaults, stabilityType: 'discrete' };
  }
  if (modelId.includes('_v3')) {
      return { ...defaults, supportsClarity: false, supportsStyle: false, supportsSpeakerBoost: false };
  }
  return defaults;
};
